...
async function loadCards()   {
  const c = await kGet('st_cards'), n = await kGet('st_nid');
  if (c) { cards = c; nextId = n || 100; return true; } return false;
}
async function saveCfg()     { await kSet('st_cfg', cfg); }
async function loadCfg()     { const v = await kGet('st_cfg'); if(v) cfg = {...cfg, ...v}; }
async function savePrec()    { await kSet('st_prec', prec); }
async function loadPrec()    {
  const v = await kGet('st_prec');
  if (v) { prec = {...prec, ...v}; if (prec.mes !== mesKey()) { prec.horas = 0; prec.mes = mesKey(); await savePrec(); } }
}
async function saveInforme() { await kSet('st_informe', informe); }
async function loadInforme() {
  const v = await kGet('st_informe');
  if (v) { informe = {...informe, ...v}; if (informe.mes !== mesKey()) { informe.participo = false; informe.cursos = 0; informe.mes = mesKey(); await saveInforme(); } }
  else { informe.mes = mesKey(); }
}
...
async function loadInformeHist() { const v = await kGet('st_informe_hist'); if(v) informeHist = v; }
...
function renderList() {
  ...
  list.innerHTML = items.map(c => {
    ...
    return '<div class="card" onclick="openDet(' + c.id + ')" oncontextmenu="event.preventDefault();shareCard(' + c.id + ')" ontouchstart="lp_start(' + c.id + ',event)" ontouchend="lp_end()" ontouchmove="lp_end()">'
      + '<div class="card-row1">'
...
async function markVisited(id) {
  ...
  // NO cambia el estado — solo registra en historial
  await saveCards(); closeDet(); updateStats(); renderList();
  toast(t('visita_reg'));
}
...
async function saveCard() {
  ...
  if (eid) {
    const i = cards.findIndex(x => x.id === parseInt(eid));
    if (i !== -1) { await cancelCard(cards[i].id); cards[i] = {...cards[i], ...d}; await schedCard(cards[i]); }
  } else {
    const nc = { id:nextId++, historial:[], ...d };
    cards.push(nc); await schedCard(nc);
  }
  await saveCards(); closeForm(); updateStats(); renderList();
...
function buildPrec() {
  ...
  const heroSection = esPrecursor
    ? '<div class="prec-big">' + prec.horas + '<span class="prec-of"> / ' + meta + 'h</span></div>'
      + '<div class="prec-pct">' + pct + '% completado</div>'
      + '<div class="prog-track"><div class="prog-fill" style="width:' + pct + '%"></div></div>'
      + (prec.ultimoRegistro
        ? '<div style="font-size:11px;color:var(--tx3);text-align:center;margin-top:6px;margin-bottom:2px">Último registro: ' + fmtDateTime(prec.ultimoRegistro) + '</div>'
        : '')
      + '<div class="prec-grid">'
  ...
  document.getElementById('precBody').innerHTML =
    ...
    + '</div>'
    + (esPrecursor
    ? '<div class="sec-lbl">' + t('prec_meta') + '</div>'
      + '<div class="sc">'
        + '<div class="sc-row"><div><div class="sc-label">' + t('auxiliar') + ' (h)</div></div><input type="number" min="1" max="300" value="' + prec.metaAux + '" oninput="setMeta(\'aux\',this.value)" style="width:70px;padding:6px 8px;border:1.5px solid var(--border);border-radius:var(--r2);font-size:14px;font-family:var(--f-sans);text-align:center"></div>'
        + '<div class="sc-row"><div><div class="sc-label">' + t('regular')  + ' (h)</div></div><input type="number" min="1" max="300" value="' + prec.metaReg + '" oninput="setMeta(\'reg\',this.value)" style="width:70px;padding:6px 8px;border:1.5px solid var(--border);border-radius:var(--r2);font-size:14px;font-family:var(--f-sans);text-align:center"></div>'
        + '<div class="sc-row"><div><div class="sc-label">' + t('especial') + ' (h)</div></div><input type="number" min="1" max="300" value="' + prec.metaEsp + '" oninput="setMeta(\'esp\',this.value)" style="width:70px;padding:6px 8px;border:1.5px solid var(--border);border-radius:var(--r2);font-size:14px;font-family:var(--f-sans);text-align:center"></div>'
      + '</div>'
      + '<button class="btn-danger" onclick="resetH()">' + t('reiniciar') + '</button>'
    : '')
}
...
function tipoOpt(v, lbl, sub) {
  ...
  return '<div class="sc-row" style="cursor:pointer" onclick="setTipo(\'' + v + '\')">'
    ...
    + '<div class="tog' + (sel?' on':'') + '" onclick="event.stopPropagation();setTipo(\'' + v + '\')"></div>'
    + '</div>';
}

async function addH(n) { 
  prec.horas = Math.round((prec.horas+n)*10)/10; 
  prec.ultimoRegistro = new Date().toISOString();
  await savePrec(); buildPrec(); toast('+' + n + 'h registrada'); 
}

async function subH() { 
  prec.horas = Math.max(0, Math.round((prec.horas-1)*10)/10); 
  prec.ultimoRegistro = new Date().toISOString();
  await savePrec(); buildPrec(); 
}


async function setTipo(v) { prec.tipo = v; await savePrec(); buildPrec(); }
...
async function resetH() { prec.horas = 0; await savePrec(); buildPrec(); toast(t('reiniciar')); }
...
function buildInforme() {
  ...
  let html =
    '<div class="inf-header">'
      + '<div class="inf-month">' + mesNombre() + '</div>'
      + '<div class="inf-sub">' + t('informe_sub') + '</div>'
    + '</div>'
...
async function enviarInforme() {
  ...
  // Archivar y marcar como enviado
...
function buildHistory() {
  document.getElementById('histBody').innerHTML =
    ...
    + '<button class="tab on"  id="htab-visitados"    onclick="setHTab(\'visitados\')">Visitados</button>'
    + '<button class="tab"     id="htab-asignaciones"  onclick="setHTab(\'asignaciones\')">Asignaciones</button>'
    + '<button class="tab"     id="htab-informes"      onclick="setHTab(\'informes\')">Informes</button>'
    + '</div>'
    + '<div id="htab-content"></div>';
...
function setHTab(v) {
  _htab = v;
  document.querySelectorAll('#histBody .tab').forEach(el => el.classList.remove('on'));
  document.getElementById('htab-' + v)?.classList.add('on');
  renderHTab(v);
}
...
function renderHTab(v) {
  ...
  if (v === 'visitados') {
    ...
    if (!completadas.length) {
      el.innerHTML = '<p style="color:var(--tx3);font-size:14px;padding:20px 0">Aún no tienes partes completadas.</p>';
      return;
    }
...
function buildSettings() {
  const c    = cfg;
  const tVal = horasToTime(c.horasAntes);
  const desc = c.horasAntes===0 ? 'Desactivado' : c.horasAntes===1 ? '1 hora antes de la visita' : c.horasAntes + ' horas antes de la visita';
  const saved = localStorage.getItem('mm_color') || '#1a2b40';

  document.getElementById('settingsBody').innerHTML =
    /* ── NOTIFICACIONES ── */
    '<div class="cfg-section-title"><svg viewBox="0 0 24 24" width="15" height="15" fill="currentColor"><path d="M12 22c1.1 0 2-.9 2-2h-4c0 1.1.9 2 2 2zm6-6v-5c0-3.07-1.64-5.64-4.5-6.32V4c0-.83-.67-1.5-1.5-1.5s-1.5.67-1.5 1.5v.68C7.63 5.36 6 7.92 6 11v5l-2 2v1h16v-1l-2-2z"/></svg>' + t('ajustes') + ' — Notificaciones</div>'
    + '<div class="cfg-card">'
      + '<div class="cfg-row"><div class="cfg-row-icon" style="background:#eef3fa"><svg viewBox="0 0 24 24" width="18" height="18" fill="#2e6be6"><path d="M12 22c1.1 0 2-.9 2-2h-4c0 1.1.9 2 2 2zm6-6v-5c0-3.07-1.64-5.64-4.5-6.32V4c0-.83-.67-1.5-1.5-1.5s-1.5.67-1.5 1.5v.68C7.63 5.36 6 7.92 6 11v5l-2 2v1h16v-1l-2-2z"/></svg></div><div class="cfg-row-info"><div class="cfg-row-label">' + t('alertas_lbl') + '</div><div class="cfg-row-sub">' + t('alertas_sub') + '</div></div><div class="tog' + (c.activo?' on':'') + '" onclick="toggleNotif(this)"></div></div>'
      + '<div class="cfg-divider"></div>'
      + '<div class="cfg-row"><div class="cfg-row-icon" style="background:#f3e5f5"><svg viewBox="0 0 24 24" width="18" height="18" fill="#7b1fa2"><path d="M0 15h2V9H0v6zm3 2h2V7H3v10zm19-8v6h2V9h-2zm-3 8h2V7h-2v10zM16.5 3h-9C6.67 3 6 3.67 6 4.5v15c0 .83.67 1.5 1.5 1.5h9c.83 0 1.5-.67 1.5-1.5v-15c0-.83-.67-1.5-1.5-1.5zM16 19H8V5h8v14z"/></svg></div><div class="cfg-row-info"><div class="cfg-row-label">' + t('vibrar') + '</div><div class="cfg-row-sub">' + t('vibrar_sub') + '</div></div><div class="tog' + (c.vibrar?' on':'') + '" onclick="toggleVibrar(this)"></div></div>'
      + '<div class="cfg-divider"></div>'
      + '<div class="cfg-row"><div class="cfg-row-icon" style="background:#e8f5e9"><svg viewBox="0 0 24 24" width="18" height="18" fill="#1e7e34"><path d="M3 9v6h4l5 5V4L7 9H3zm13.5 3c0-1.77-1.02-3.29-2.5-4.03v8.05c1.48-.73 2.5-2.25 2.5-4.02zM14 3.23v2.06c2.89.86 5 3.54 5 6.71s-2.11 5.85-5 6.71v2.06c4.01-.91 7-4.49 7-8.77s-2.99-7.86-7-8.77z"/></svg></div><div class="cfg-row-info"><div class="cfg-row-label">' + t('sonido') + '</div><div class="cfg-row-sub">' + t('sonido_sub') + '</div></div><div class="tog' + (c.sonido?' on':'') + '" onclick="toggleSonido(this)"></div></div>'
      + '<div class="cfg-divider"></div>'
      + '<div class="cfg-row"><div class="cfg-row-icon" style="background:#fff8ee"><svg viewBox="0 0 24 24" width="18" height="18" fill="#a0660a"><path d="M11.99 2C6.47 2 2 6.48 2 12s4.47 10 9.99 10C17.52 22 22 17.52 22 12S17.52 2 11.99 2zM12 20c-4.42 0-8-3.58-8-8s3.58-8 8-8 8 3.58 8 8-3.58 8-8 8zm.5-13H11v6l5.25 3.15.75-1.23-4.5-2.67V7z"/></svg></div><div class="cfg-row-info"><div class="cfg-row-label">' + t('notif_lbl') + '</div><div class="cfg-row-sub" id="notifDesc">' + desc + '</div></div></div>'
      + '<div class="notif-native-wrap"><input type="time" id="notifTimeInput" value="' + tVal + '" onchange="onNotifTimeChange(this.value)"/><div class="notif-native-hint">' + t('toca_hora') + '</div></div>'
      + '<div class="cfg-divider"></div>'
      + '<div class="cfg-row cfg-row-tap" onclick="reqPermission()"><div class="cfg-row-icon" style="background:#eef3fa"><svg viewBox="0 0 24 24" width="18" height="18" fill="#2e6be6"><path d="M12 22c1.1 0 2-.9 2-2h-4c0 1.1.9 2 2 2zm6-6v-5c0-3.07-1.64-5.64-4.5-6.32V4c0-.83-.67-1.5-1.5-1.5s-1.5.67-1.5 1.5v.68C7.63 5.36 6 7.92 6 11v5l-2 2v1h16v-1l-2-2z"/></svg></div><div class="cfg-row-info"><div class="cfg-row-label">🔔 ' + t('notif_activar') + '</div></div><svg class="cfg-chev" viewBox="0 0 24 24" width="16" height="16" fill="currentColor"><path d="M10 6L8.59 7.41 13.17 12l-4.58 4.59L10 18l6-6z"/></svg></div>'
    + '</div>'

    /* ── DATOS ── */
    + '<div class="cfg-section-title"><svg viewBox="0 0 24 24" width="15" height="15" fill="currentColor"><path d="M20 6h-8l-2-2H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V8c0-1.1-.9-2-2-2z"/></svg>Datos</div>'
    + '<div class="cfg-card">'
      + '<div class="cfg-row cfg-row-tap" onclick="exportJSON()"><div class="cfg-row-icon" style="background:#edf7ef"><svg viewBox="0 0 24 24" width="18" height="18" fill="#1e7e34"><path d="M19 9h-4V3H9v6H5l7 7 7-7zm-8 2V5h2v6h1.17L12 13.17 9.83 11H11zm-6 7h14v2H5z"/></svg></div><div class="cfg-row-info"><div class="cfg-row-label">' + t('guardar_datos') + '</div><div class="cfg-row-sub">' + t('guardar_datos_sub') + '</div></div><svg class="cfg-chev" viewBox="0 0 24 24" width="16" height="16" fill="currentColor"><path d="M10 6L8.59 7.41 13.17 12l-4.58 4.59L10 18l6-6z"/></svg></div>'
      + '<div class="cfg-divider"></div>'
      + '<div class="cfg-row cfg-row-tap" onclick="triggerJSON()"><div class="cfg-row-icon" style="background:#eef3fa"><svg viewBox="0 0 24 24" width="18" height="18" fill="#2e6be6"><path d="M9 16h6v-6h4l-7-7-7 7h4v6zm-4 2h14v2H5z"/></svg></div><div class="cfg-row-info"><div class="cfg-row-label">' + t('agregar_datos') + '</div><div class="cfg-row-sub">' + t('agregar_datos_sub') + '</div></div><svg class="cfg-chev" viewBox="0 0 24 24" width="16" height="16" fill="currentColor"><path d="M10 6L8.59 7.41 13.17 12l-4.58 4.59L10 18l6-6z"/></svg></div>'
    + '</div>'
    + '<div class="cfg-card cfg-card-danger"><div class="cfg-row cfg-row-tap" onclick="borrarTodo()"><div class="cfg-row-icon" style="background:#fef0f2"><svg viewBox="0 0 24 24" width="18" height="18" fill="#9b2335"><path d="M6 19c0 1.1.9 2 2 2h8c1.1 0 2-.9 2-2V7H6v12zM19 4h-3.5l-1-1h-5l-1 1H5v2h14V4z"/></svg></div><div class="cfg-row-info"><div class="cfg-row-label" style="color:var(--red)">' + t('borrar_datos') + '</div><div class="cfg-row-sub">' + t('borrar_sub') + '</div></div><svg class="cfg-chev" viewBox="0 0 24 24" width="16" height="16" fill="var(--red)" style="opacity:.5"><path d="M10 6L8.59 7.41 13.17 12l-4.58 4.59L10 18l6-6z"/></svg></div></div>'

    /* ── ORDENAR ── */
    + '<div class="cfg-section-title"><svg viewBox="0 0 24 24" width="14" height="14" fill="currentColor"><path d="M3 18h6v-2H3v2zM3 6v2h18V6H3zm0 7h12v-2H3v2z"/></svg>' + t('orden_lbl') + '</div>'
    + '<div class="cfg-card">'
      + '<div class="cfg-row cfg-row-tap" onclick="setOrden(\'fecha\')"><div class="cfg-row-icon" style="background:#e8f5e9"><svg viewBox="0 0 24 24" width="18" height="18" fill="#1e7e34"><path d="M19 3h-1V1h-2v2H8V1H6v2H5c-1.11 0-1.99.9-1.99 2L3 19c0 1.1.89 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm0 16H5V8h14v11zM7 10h5v5H7z"/></svg></div><div class="cfg-row-info"><div class="cfg-row-label">' + t('por_fecha') + '</div><div class="cfg-row-sub">' + t('por_fecha_sub') + '</div></div>' + (c.orden==='fecha'?'<svg viewBox="0 0 24 24" width="18" height="18" fill="var(--navy)"><path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z"/></svg>':'') + '</div>'
      + '<div class="cfg-divider"></div>'
      + '<div class="cfg-row cfg-row-tap" onclick="setOrden(\'nombre\')"><div class="cfg-row-icon" style="background:#eef3fa"><svg viewBox="0 0 24 24" width="18" height="18" fill="#2e6be6"><path d="M11 18h2v-2h-2v2zm1-16C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8zm0-14c-2.21 0-4 1.79-4 4h2c0-1.1.9-2 2-2s2 .9 2 2c0 2-3 1.75-3 5h2c0-2.25 3-2.5 3-5 0-2.21-1.79-4-4-4z"/></svg></div><div class="cfg-row-info"><div class="cfg-row-label">' + t('por_nombre') + '</div><div class="cfg-row-sub">' + t('por_nombre_sub') + '</div></div>' + (c.orden==='nombre'?'<svg viewBox="0 0 24 24" width="18" height="18" fill="var(--navy)"><path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z"/></svg>':'') + '</div>'
    + '</div>'

    /* ── IDIOMA ── */
    + '<div class="cfg-section-title"><svg viewBox="0 0 24 24" width="14" height="14" fill="currentColor"><path d="M11.99 2C6.47 2 2 6.48 2 12s4.47 10 9.99 10C17.52 22 22 17.52 22 12S17.52 2 11.99 2zm6.93 6h-2.95c-.32-1.25-.78-2.45-1.38-3.56 1.84.63 3.37 1.9 4.33 3.56zm-6.92-3.96c.83 1.2 1.48 2.53 1.91 3.96h-3.82c.43-1.43 1.08-2.76 1.91-3.96zM4.26 14C4.1 13.36 4 12.69 4 12s.1-1.36.26-2h3.38c-.08.66-.14 1.32-.14 2s.06 1.34.14 2H4.26zm.82 2h2.95c.32 1.25.78 2.45 1.38 3.56-1.84-.63-3.37-1.9-4.33-3.56zm2.95-8H5.08c.96-1.66 2.49-2.93 4.33-3.56C8.81 5.55 8.35 6.75 8.03 8zM12 19.96c-.83-1.2-1.48-2.53-1.91-3.96h3.82c-.43 1.43-1.08 2.76-1.91 3.96zM14.34 14H9.66c-.09-.66-.16-1.32-.16-2s.07-1.35.16-2h4.68c.09.65.16 1.32.16 2s-.07 1.34-.16 2zm.25 5.56c.6-1.11 1.06-2.31 1.38-3.56h2.95c-.96 1.66-2.49 2.93-4.33 3.56zM16.36 14c.08-.66.14-1.32.14-2s-.06-1.34-.14-2h3.38c.16.64.26 1.31.26 2s-.1 1.36-.26 2h-3.38z"/></svg>' + t('idioma_lbl') + '</div>'
    + '<div class="cfg-card">' + buildLangPicker() + '</div>'

    /* ── PERSONALIZACIÓN ── */
    + '<div class="cfg-section-title"><svg viewBox="0 0 24 24" width="15" height="15" fill="currentColor"><path d="M12 3c-4.97 0-9 4.03-9 9s4.03 9 9 9c.83 0 1.5-.67 1.5-1.5 0-.39-.15-.74-.39-1.01-.23-.26-.38-.61-.38-.99 0-.83.67-1.5 1.5-1.5H16c2.76 0 5-2.24 5-5 0-4.42-4.03-8-9-8zm-5.5 9c-.83 0-1.5-.67-1.5-1.5S5.67 9 6.5 9 8 9.67 8 10.5 7.33 12 6.5 12zm3-4C8.67 8 8 7.33 8 6.5S8.67 5 9.5 5s1.5.67 1.5 1.5S10.33 8 9.5 8zm5 0c-.83 0-1.5-.67-1.5-1.5S13.67 5 14.5 5s1.5.67 1.5 1.5S15.33 8 14.5 8zm3 4c-.83 0-1.5-.67-1.5-1.5S16.67 9 17.5 9s1.5.67 1.5 1.5-.67 1.5-1.5 1.5z"/></svg>' + t('personalizacion_lbl') + '</div>'
    + '<div class="cfg-card" id="colorCard">'
      + '<div class="cfg-row cfg-row-tap" onclick="togglePalette()"><div id="colorSwatch" style="width:36px;height:36px;border-radius:9px;flex-shrink:0;border:2px solid rgba(0,0,0,.1);background:' + saved + '"></div><div class="cfg-row-info"><div class="cfg-row-label">' + t('color_tema') + '</div><div class="cfg-row-sub" id="colorName">' + getColorName(saved) + '</div></div><svg id="colorChev" viewBox="0 0 24 24" width="18" height="18" fill="currentColor" style="color:var(--tx3);transition:transform .25s;flex-shrink:0"><path d="M7.41 8.59L12 13.17l4.59-4.58L18 10l-6 6-6-6z"/></svg></div>'
      + '<div class="pal-drawer" id="palDrawer">'
        + buildPaletteHTML()
        + '<div class="pal-custom"><div class="pal-group-lbl">' + t('color_custom') + '</div>'
          + '<label class="pal-custom-btn"><span id="customSwatch" style="width:30px;height:30px;border-radius:50%;background:' + saved + ';border:3px solid rgba(0,0,0,.15);flex-shrink:0"></span><span style="flex:1;font-size:13px;color:var(--tx2)">' + t('color_custom') + '</span><input type="color" id="colorPicker" value="' + saved + '" oninput="applyThemeColor(this.value)" onchange="saveThemeColor(this.value,\'Personalizado\')" style="position:absolute;opacity:0;width:100%;height:100%;cursor:pointer;top:0;left:0"></label>'
        + '</div>'
      + '</div>'
    + '</div>';
}
...
async function loadAsig() {
  const v = await kGet('st_asig');
  if (v) { asignaciones = v.list || []; nextAsigId = v.nid || 1; }
}
...
function buildAsignaciones() {
  ...
  html += '<div style="padding:16px 16px 0">'
    + '<button onclick="goToAgregarAsignacion()" style="'
      + 'width:100%;padding:22px 16px;border-radius:14px;'
      + 'border:2px dashed #7ab8c8;background:#f0f8fb;'
  ...
  if (pendientes.length > 0) {
    SECCIONES_INFO.forEach(sec => {
      ...
      html += '<div style="margin:16px 16px 0;border-radius:14px;overflow:hidden;border:1.5px solid ' + sec.color + '20">'
        + '<div style="background:' + sec.bg + ';padding:10px 14px;display:flex;align-items:center;gap:10px">'
      ...
      items.forEach(a => {
        ...
        html += '<div onclick="openAsigDet(' + a.id + ')" style="'
          ...
          + 'background:#fff;border-top:1px solid var(--border);cursor:pointer">'
          + '<div style="flex:1">'
...
function goToAgregarAsignacion() {
  ...
  const SECCIONES = [
    { id:'tesoros', img:'img/tesoros.png', color:'#2e7d8a', bg:'#eef7f8', tipos:['discurso10','perlas','lectura'] },
    { id:'maestros', img:'img/maestros.png', color:'#a0660a', bg:'#fff8ee', tipos:['conversacion','revisitas','discipulos','discurso'] },
    { id:'cristiana',img:'img/cristiana.png',color:'#8b1a1a', bg:'#fdf0f0', tipos:[] },
  ];
  ...
  SECCIONES.forEach(sec => {
    html += '<div style="border-radius:16px;overflow:hidden;border:1.5px solid ' + sec.color + '20">'
      + '<div style="background:' + sec.bg + ';padding:14px 16px;display:flex;align-items:center;gap:12px">'
    ...
    tiposPersonalizados.filter(t => t.seccion === sec.id).forEach(tipo => {
      html += '<div style="display:flex;align-items:center;border-top:1px solid var(--border);background:#fff">'
    ...
    html += '<button onclick="abrirFormNuevoTipo(\'' + sec.id + '\')" style="'
      ...
      + 'width:100%;padding:13px 16px;background:' + sec.bg + ';'
...
async function saveAsig_() {
  ...
  if (_asigEdit) {
    ...
    if (i !== -1) {
      asignaciones[i] = { ...asignaciones[i], ...data };
      await cancelAsigNotifs(_asigEdit.id);
      await schedAsigNotifs(asignaciones[i]);
    }
  } else {
    const na = { id: nextAsigId++, ...data };
...
async function deleteAsig(id) {
  const tipo = asignaciones.find(x => x.id === id)?.tipo;
  try { await cancelAsigNotifs(id); } catch(e){}
...
async function init() {
  ...
  await registerSW();
  await schedAll();
  await schedInformeNotif(); // ← aquí


  window.addEventListener('mm:notification-tapped', function(e) {
    const cardId = e.detail.cardId;
    if (cardId && cardId.startsWith('asig-')) {
